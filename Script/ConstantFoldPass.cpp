...
namespace RealSix::Script
{
	...
	Stmt *ConstantFoldPass::ExecuteEnumDecl(EnumDecl *decl)
	{
	...
	Stmt *ConstantFoldPass::ExecuteVarDecl(VarDecl *decl)
	{
	...
	Stmt *ConstantFoldPass::ExecuteFunctionDecl(FunctionDecl *decl)
	{
	...
	Stmt *ConstantFoldPass::ExecuteClassDecl(ClassDecl *decl)
	{
	...
	Stmt *ConstantFoldPass::ExecuteModuleDecl(ModuleDecl *decl)
	{
		return decl;
	}
...
#undef BIN_EXPR
		...
		else if (expr->kind == AstKind::PREFIX)
		{
			...
			if (prefix->right->kind == AstKind::LITERAL)
			{
				...
				if (rightLiteralExpr->type.IsFloating() && prefix->op == "-")
				{
					auto numExpr = new LiteralExpr(prefix->tagToken, -rightLiteralExpr->f64Value);
					SAFE_DELETE(prefix);
					return numExpr;
				}
				else if (rightLiteralExpr->type.IsInteger() && prefix->op == "-")
				{
					auto numExpr = new LiteralExpr(prefix->tagToken, -rightLiteralExpr->i64Value);
					SAFE_DELETE(prefix);
					return numExpr;
				}
				else if (rightLiteralExpr->type.Is(TypeKind::BOOL) && prefix->op == "!")
				{
