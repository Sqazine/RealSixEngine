...
namespace RealSix::Script
{
	...
	void Compiler::CompileEnumDecl(EnumDecl *decl)
	{
		...
		for (const auto &[k, v] : decl->enumItems)
		{
			...
			if (v->kind == AstKind::LITERAL)
			{
				...
				REALSIX_SCRIPT_LOG_ERROR(v->tagToken, "Enum value only integer num,floating point num,boolean or string is available.");
				...
				else
				{
					if (expr->right->kind == AstKind::ARRAY)
					{
						...
						if (assigneeElements.size() < rightElements.size())
							REALSIX_SCRIPT_LOG_WARN(rightElements[assigneeElements.size()]->tagToken, "variable count less than value count,ignore the rest value");
						else if (assigneeElements.size() > rightElements.size())
							REALSIX_SCRIPT_LOG_WARN(assigneeElements[rightElements.size()]->tagToken, "variable count greater than value count,rest value set as null");
					}
				}
	...
	void Compiler::CompilePrefixExpr(PrefixExpr *expr)
	{
		...
		{
			while (expr->right->kind == AstKind::PREFIX && ((PrefixExpr *)expr->right)->op == "++" || ((PrefixExpr *)expr->right)->op == "--")
		...
		else if (expr->op == "--")
		{
			...
			REALSIX_SCRIPT_LOG_ERROR(expr->tagToken, "No prefix op:{}", expr->op);
	}
	void Compiler::CompilePostfixExpr(PostfixExpr *expr, const RWState &state, bool isDelayCompile)
	{
		...
		if (!isDelayCompile)
		{
			...
			if (expr->op == "++")
				...
				REALSIX_SCRIPT_LOG_ERROR(expr->tagToken, "No postfix op:{}", expr->op);
			CompileExpr(expr->left, RWState::WRITE);
	...
	{
		auto identExpr = new IdentifierExpr(expr->tagToken, "this");
	...
	void Compiler::CompileIdentifierExpr(IdentifierExpr *expr, const RWState &state, int8_t paramCount)
	{
		...
		if (state == RWState::WRITE)
		{
			if (symbol.permission == Permission::MUTABLE)
			{
				...
				REALSIX_SCRIPT_LOG_ERROR(expr->tagToken, "{} is a constant,which cannot be assigned!", expr->ToString());
		}
		else
		{
...

		{
			for (const auto &[k, v] : decl->variables)
			{
				...
				else if (k->kind == AstKind::VAR_DESC)
				{
					...
					REALSIX_SCRIPT_LOG_ERROR(k->tagToken, "Unknown variable:{}", k->ToString());
			}
		}
