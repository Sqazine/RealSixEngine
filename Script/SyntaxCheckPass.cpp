...
namespace RealSix::Script
{
    ...
    Stmt *SyntaxCheckPass::ExecuteVarDecl(VarDecl *decl)
    {
            ...
            if (k->kind == AstKind::ARRAY)
            {
                ...
                for (int32_t i = 0; i < arrayK->elements.size(); ++i)
                {
                    ...
                    if (e->kind != AstKind::VAR_DESC && e->kind != AstKind::VAR_ARG)
                        REALSIX_LOG_ERROR_WITH_LOC(e->tagToken, TEXT("only variable description or variable argument is available in destructing assign expr."));

                    if (e->kind == AstKind::VAR_ARG && i != arrayK->elements.size() - 1)
                        REALSIX_LOG_ERROR_WITH_LOC(k->tagToken, TEXT("variable argument expr only available at the end of destructing assignment expr."));
                }
            }
            else if (k->kind == AstKind::VAR_DESC)
                k = ExecuteExpr(k);
            else
                REALSIX_LOG_ERROR_WITH_LOC(k->tagToken, TEXT("only destructing assign expr or variable description is available in let/const stmt's binding part."));
    ...
    Stmt *SyntaxCheckPass::ExecuteEnumDecl(EnumDecl *decl)
    {
    ...
    Stmt *SyntaxCheckPass::ExecuteFunctionDecl(FunctionDecl *decl)
    {
        for (int32_t i = 0; i < decl->parameters.size(); ++i)
        {
            ...
            if (e->name->kind == AstKind::VAR_ARG && i != decl->parameters.size() - 1)
                REALSIX_LOG_ERROR_WITH_LOC(e->tagToken, TEXT("variable argument expr only available at the end of function parameter list."));
        }

        decl->body = (ScopeStmt *)ExecuteScopeStmt(decl->body);
        return decl;
    }
    Stmt *SyntaxCheckPass::ExecuteClassDecl(ClassDecl *decl)
    {
        return decl;
    }
    ...
    Stmt *SyntaxCheckPass::ExecuteModuleDecl(ModuleDecl *decl)
    {
        return decl;
    }
    ...
    Expr *SyntaxCheckPass::ExecuteDictExpr(DictExpr *expr)
    {
        for (auto &[k, v] : expr->elements)
        {
            if (!IsConstantLiteral(k))
                REALSIX_LOG_ERROR_WITH_LOC(k->tagToken, TEXT("Dict keys can only have constant literals (int num,real num,string,boolean,null or variable identifier)."));
            v = ExecuteExpr(v);
        }
        return expr;
    }
    ...
    Expr *SyntaxCheckPass::ExecuteNewExpr(NewExpr *expr)
    {
        if (expr->callee->kind != AstKind::CALL)
            REALSIX_LOG_ERROR_WITH_LOC(expr->callee->tagToken, TEXT("Not a valid new expr,call expr is necessary followed 'new' keyword."));
    ...
    Expr *SyntaxCheckPass::ExecuteLambdaExpr(LambdaExpr *expr)
    {
        for (int32_t i = 0; i < expr->parameters.size(); ++i)
        {
            ...
            if (e->name->kind == AstKind::VAR_ARG && i != expr->parameters.size() - 1)
                REALSIX_LOG_ERROR_WITH_LOC(e->tagToken, TEXT("variable argument expr only available at the end of lambda parameter list."));
        }
    ...
    Expr *SyntaxCheckPass::ExecuteRefExpr(RefExpr *expr)
    {
        if (expr->refExpr->kind != AstKind::IDENTIFIER && expr->refExpr->kind != AstKind::INDEX)
            REALSIX_LOG_ERROR_WITH_LOC(expr->tagToken, TEXT("Only left value is available for reference."));
        return expr;
    }
    ...
    Expr *SyntaxCheckPass::ExecuteVarDescExpr(VarDescExpr *expr)
    {
        if (expr->name->kind != AstKind::IDENTIFIER && expr->name->kind != AstKind::VAR_ARG)
            REALSIX_LOG_ERROR_WITH_LOC(expr->tagToken, TEXT("Only identifier or variable argument is available at variable declaration or param declaration"));
        return expr;
    }
