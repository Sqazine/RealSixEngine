...
namespace RealSix::Script
{
		...
		while (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			else if (IsMatchCurTokenAndStepOnce(TokenKind::FUNCTION))
			{
				...
				if (fn->name->literal == classStmt->name)
					REALSIX_LOG_ERROR_WITH_LOC(fn->name->tagToken, TEXT("The class member function name :{} conflicts with its class:{}, only constructor function name is allowed to same with its class's name"), fn->name->literal);
	...
	Stmt *Parser::ParseModuleDecl()
	{
		...
		while (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			case TokenKind::MODULE:
				...
				break;
			default:
				REALSIX_LOG_ERROR_WITH_LOC(GetCurToken(), TEXT("Only let,const,function,class,enum and module is available in module scope"));
			}
		}
	...
	Expr *Parser::ParseThisExpr()
	{
		...
		if (!mCurClassInfo)
			REALSIX_LOG_ERROR_WITH_LOC(thisExpr->tagToken, TEXT("Invalid 'this' keyword:Cannot use 'this' outside class."));
	...
	Expr *Parser::ParseBaseExpr()
	{
		...
		if (!mCurClassInfo)
			REALSIX_LOG_ERROR_WITH_LOC(baseExpr->tagToken, TEXT("Invalid 'base' keyword:Cannot use 'base' outside class."));
	...
	Expr *Parser::ParseMatchExpr()
	{
		...
		if (!IsMatchCurToken(TokenKind::RBRACE))
		{
			do
			{
				...
				{
					if (hasDefaultBranch)
						REALSIX_LOG_ERROR_WITH_LOC(GetCurToken(), TEXT("Already exists a default branch.only a default branch is available in a match expr."));
	...
	Expr *Parser::ParseCompoundExpr()
	{
		...
		do
		{
			if (IsMatchCurToken(TokenKind::RBRACE_RPAREN))
				REALSIX_LOG_ERROR_WITH_LOC(GetCurToken(), TEXT("Expr required at the end of block expression."));
		...
		if (stmts.back()->kind != AstKind::EXPR)
			REALSIX_LOG_ERROR_WITH_LOC(stmts.back()->tagToken, TEXT("Expr required at the end of block expression."));
	...
	Expr *Parser::ParseExpr(Precedence precedence)
	{
		if (mPrefixFunctions.find(GetCurToken()->kind) == mPrefixFunctions.end())
		{
			...
			REALSIX_LOG_ERROR_WITH_LOC(token, TEXT("no prefix definition for:{}"), token->literal);
	...
	Expr *Parser::ParseLiteralExpr()
	{
		...
		REALSIX_LOG_ERROR_WITH_LOC(token, TEXT("Unknown Literal."));
		return nullptr;
	}
		...
		if (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			do
			{
				...
				if (key->kind != AstKind::IDENTIFIER)
					REALSIX_LOG_ERROR_WITH_LOC(key->tagToken, TEXT("Struct object require key must be a valid identifier."));
	...
	std::pair<Expr *, Expr *> Parser::ParseDestructuringAssignmentExpr()
	{
		...
		do
		{
			...
			if (varDescExpr->name->kind == AstKind::VAR_ARG) // check if has var arg
			{
				varArgCount++;
				if (varArgCount > 1)
					REALSIX_LOG_ERROR_WITH_LOC(varDescExpr->tagToken, TEXT("only 1 variable arg decl is available in var declaration."));
			}
			...
			for (int32_t i = 0; i < arrayExpr->elements.size(); ++i)
			{
				if (((VarDescExpr *)arrayExpr->elements[i])->name->kind == AstKind::VAR_ARG)
				{
					if (i < arrayExpr->elements.size() - 1)
						REALSIX_LOG_ERROR_WITH_LOC(arrayExpr->elements[i]->tagToken, TEXT("variable arg decl must be located at the end of destructing assignment declaration."));
				}
			}
		}
		...
		if (IsMatchCurTokenAndStepOnce(TokenKind::EQUAL))
		{
			...
			if (value->kind == AstKind::ARRAY)
			{
				...
				else if (grad > 0)
				{
					...
					REALSIX_LOG_ERROR_WITH_LOC(GetCurToken(), TEXT("variable less than value."));
			}
	...
	std::vector<Expr *> Parser::ParseSwitchOrMatchConditionExprList(Expr* expr)
	{
		...
		do
		{
			...
			auto caseCondition = new InfixExpr(GetCurToken(), TEXT("=="), expr, valueCompareExpr);
	...
	Token *Parser::Consume(TokenKind kind, STRING_VIEW errMsg)
	{
		if (mSkippingConsumeTokenKindStack.empty() || kind != mSkippingConsumeTokenKindStack.back())
		{
			...
			REALSIX_LOG_ERROR_WITH_LOC(token, TEXT("{}"), errMsg);
		}
		return nullptr;
	}
