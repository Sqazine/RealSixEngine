...
namespace RealSix::Script
{
	...
	Stmt *Parser::ParseVarDecl()
	{
		...
		Consume(kind, "Expect 'let' or 'const' key word");
		...
		Consume(TokenKind::SEMICOLON, "Expect ';' after let or const declaration.");
	...
	Stmt *Parser::ParseFunctionDecl()
	{
...

		{
			...
			Consume(TokenKind::LPAREN, "Expect '(' after 'fn' keyword");
			...
			Consume(TokenKind::RPAREN, "Expect ')' after function stmt's '('");
	...
	Stmt *Parser::ParseClassDecl()
	{
		...
		Consume(TokenKind::LBRACE, "Expect '{' after class name or parent class name");
		...
		Consume(TokenKind::RBRACE, "Expect '}' after class stmt's '{'");
	...
	Stmt *Parser::ParseEnumDecl()
	{
		...
		Consume(TokenKind::ENUM, "Expect 'enum' keyword.");
		enumStmt->name = (IdentifierExpr *)ParseIdentifierExpr();
		Consume(TokenKind::LBRACE, "Expect '{' after 'enum' keyword.");
		...
		while (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			if (IsMatchCurToken(TokenKind::COMMA))
				Consume(TokenKind::COMMA, "Expect ',' after enum item.");
		}

		Consume(TokenKind::RBRACE, "Expect '}' at the end of the 'enum' stmt.");
	...
	{
		auto token = Consume(TokenKind::MODULE, "Expect 'module' keyword.");

		auto moduleDecl = new ModuleDecl(token);

		moduleDecl->name = (IdentifierExpr *)ParseIdentifierExpr();

		Consume(TokenKind::LBRACE, "Expect '{' after module name.");
	...
	Stmt *Parser::ParseExprStmt()
	{
		...
		Consume(TokenKind::SEMICOLON, "Expect ';' after expr stmt.");
		return exprStmt;
	}
	...
	Stmt *Parser::ParseReturnStmt()
	{
		...
		Consume(TokenKind::RETURN, "Expect 'return' key word.");
		...
		Consume(TokenKind::SEMICOLON, "Expect ';' after return stmt");
		return returnStmt;
	}
	...
	Stmt *Parser::ParseIfStmt()
	{
		...
		Consume(TokenKind::IF, "Expect 'if' key word.");
	...
	Stmt *Parser::ParseScopeStmt()
	{
		...
		Consume(TokenKind::LBRACE, "Expect '{'.");
		while (!IsMatchCurToken(TokenKind::RBRACE))
	...
	Stmt *Parser::ParseWhileStmt()
	{
		...
		Consume(TokenKind::WHILE, "Expect 'while' keyword.");
		Consume(TokenKind::LPAREN, "Expect '(' after 'while'.");

		whileStmt->condition = ParseExpr();

		Consume(TokenKind::RPAREN, "Expect ')' after while stmt's condition.");
	...
	Stmt *Parser::ParseForStmt()
	{
		...
		Consume(TokenKind::FOR, "Expect 'for' keyword.");
		Consume(TokenKind::LPAREN, "Expect '(' after 'for'.");
		...
		else
		{
			...
			Consume(TokenKind::SEMICOLON, "Expect ';' after for stmt's initializer stmt");
		}

		Expr *condition = nullptr;
		if (!IsMatchCurToken(TokenKind::SEMICOLON))
			condition = ParseExpr();
		Consume(TokenKind::SEMICOLON, "Expect ';' after for stmt's condition expr.");
		...
		Consume(TokenKind::RPAREN, "Expect ')' after for stmt's increment expr(s)");
	...
	Stmt *Parser::ParseBreakStmt()
	{
		...
		Consume(TokenKind::BREAK, "Expect 'break' keyword.");
		Consume(TokenKind::SEMICOLON, "Expect ';' after 'break' keyword.");
		return breakStmt;
	}
	...
	Stmt *Parser::ParseContinueStmt()
	{
		...
		Consume(TokenKind::CONTINUE, "Expect 'continue' keyword");
		Consume(TokenKind::SEMICOLON, "Expect ';' after 'continue' keyword.");
		return continueStmt;
	}
	...
	Stmt *Parser::ParseSwitchStmt()
	{
		...
		Consume(TokenKind::SWITCH, "Expect 'switch' keyword.");
		Consume(TokenKind::LPAREN, "Expect '(' after 'switch' keyword.");

		auto switchExpr = ParseIdentifierExpr();

		Consume(TokenKind::RPAREN, "Expect ')' after switch's expression.");
		Consume(TokenKind::LBRACE, "Expect '{' after 'switch' keyword.");
		...
		while (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			{
				Consume(TokenKind::COLON, "Expect ':' after condition expr.");
				...
				if (IsMatchCurTokenAndStepOnce(TokenKind::LBRACE))
				{
					...
					Consume(TokenKind::RBRACE, "Expect '}' at the end of default block while has multiple statement");
				}
				else
					defaultScopeStmt->stmts.emplace_back(ParseStmt());
			}
			...
			{
				std::vector<Expr *> conditions = ParseSwitchOrMatchConditionExprList(switchExpr);
				...
				if (IsMatchCurTokenAndStepOnce(TokenKind::LBRACE))
				{
					...
					Consume(TokenKind::RBRACE, "Expect '}' at the end of block while has multiple statements.");
				}
				else
					item.second->stmts.emplace_back(ParseStmt());
		...
		Consume(TokenKind::RBRACE, "Expect '}' after switch stmt");
			...
			for (size_t i = 0; i < items.size(); ++i)
			{
				...
				else
				{
					...
					for (size_t j = 1; j < items[i].first.size(); ++j)
						condition = new InfixExpr(condition->tagToken, "||", condition, items[i].first[j]);
	...
	Expr *Parser::ParseLambdaExpr()
	{
		...
		Consume(TokenKind::FUNCTION, "Expect 'fn' keyword");
		Consume(TokenKind::LPAREN, "Expect '(' after 'fn' keyword");
		...
		Consume(TokenKind::RPAREN, "Expect ')' after lambda expr's '('");
	...
	Expr *Parser::ParseNewExpr()
	{
		...
		Consume(TokenKind::NEW, "Expect 'new' keyword");
	...
	{
		auto token = Consume(TokenKind::THIS, "Expect 'this' keyword");

		auto thisExpr = new ThisExpr(token);

		if (!mCurClassInfo)
			REALSIX_SCRIPT_LOG_ERROR(thisExpr->tagToken, "Invalid 'this' keyword:Cannot use 'this' outside class.");
	...
	{
		auto token = Consume(TokenKind::BASE, "Expect 'base' keyword");

		Consume(TokenKind::DOT, "Expect '.' after base keyword");

		auto baseExpr = new BaseExpr(token, (IdentifierExpr *)ParseIdentifierExpr());

		if (!mCurClassInfo)
			REALSIX_SCRIPT_LOG_ERROR(baseExpr->tagToken, "Invalid 'base' keyword:Cannot use 'base' outside class.");
	...
	Expr *Parser::ParseMatchExpr()
	{
		...
		auto matchToken = Consume(TokenKind::MATCH, "Expect 'match' keyword.");
		Consume(TokenKind::LPAREN, "Expect '(' after 'match' keyword.");

		auto judgeExpr = ParseExpr();

		Consume(TokenKind::RPAREN, "Expect ')' after match's expression.");
		Consume(TokenKind::LBRACE, "Expect '{' after 'match' keyword.");
		...
		if (!IsMatchCurToken(TokenKind::RBRACE))
		{
			do
			{
				...
				{
					if (hasDefaultBranch)
						REALSIX_SCRIPT_LOG_ERROR(GetCurToken(), "Already exists a default branch.only a default branch is available in a match expr.");

					GetCurTokenAndStepOnce();

					Consume(TokenKind::COLON, "Expect ':' after default's condition expr.");
				...
				else
				{
					...
					std::vector<Expr *> conditions = ParseSwitchOrMatchConditionExprList(judgeExpr);
		...
		Consume(TokenKind::RBRACE, "Expect '}' after match expr");
		...
		for (int32_t i = 0; i < items.size(); ++i)
		{
			...
			else
			{
				...
				for (size_t j = 1; j < items[i].first.size(); ++j)
					fullCondition = new InfixExpr(fullCondition->tagToken, "||", fullCondition, items[i].first[j]);
	...
	{
		auto token = Consume(TokenKind::LPAREN_LBRACE, "Expect '({'.");
		...
		do
		{
			if (IsMatchCurToken(TokenKind::RBRACE_RPAREN))
				REALSIX_SCRIPT_LOG_ERROR(GetCurToken(), "Expr required at the end of block expression.");
		...
		if (stmts.back()->kind != AstKind::EXPR)
			REALSIX_SCRIPT_LOG_ERROR(stmts.back()->tagToken, "Expr required at the end of block expression.");
		...
		Consume(TokenKind::RBRACE_RPAREN, "Expect '})'.");
	...
	Expr *Parser::ParseExpr(Precedence precedence)
	{
		if (mPrefixFunctions.find(GetCurToken()->kind) == mPrefixFunctions.end())
		{
			...
			REALSIX_SCRIPT_LOG_ERROR(token, "no prefix definition for:{}", token->literal);
	...
	{
		auto token = Consume(TokenKind::IDENTIFIER, "Unexpect Identifier'" + GetCurToken()->literal + "'.");
	...
	Expr *Parser::ParseLiteralExpr()
	{
		...
		if (token->kind == TokenKind::NUMBER)
		{
			...
			if (!literal.IsAtLast(literal.Find('.')))
			{
				auto v = literal.ToDouble();
				numExpr->f64Value = v;
		...
		REALSIX_SCRIPT_LOG_ERROR(token, "Unknown Literal.");
		return nullptr;
	}

	Expr *Parser::ParseGroupExpr()
	{
		auto token = Consume(TokenKind::LPAREN, "Expect '('.");
	...
	{
		auto token = Consume(TokenKind::LBRACKET, "Expect '['.");
		...
		Consume(TokenKind::RBRACKET, "Expect ']'.");
		return arrayExpr;
	}

	Expr *Parser::ParseDictExpr()
	{
		auto token = Consume(TokenKind::LBRACE, "Expect '{'.");
		...
		if (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			do
			{
				...
				Expr *key = ParseExpr();
				Consume(TokenKind::COLON, "Expect ':' after dict key.");
		...
		Consume(TokenKind::RBRACE, "Expect '}' after dict.");
		return dictExpr;
	}

	Expr *Parser::ParseStructExpr()
	{
		auto token = Consume(TokenKind::STRUCT, "Expect 'struct' keyword.");
		Consume(TokenKind::LBRACE, "Expect '{' after 'struct' keyword.");
		...
		if (!IsMatchCurToken(TokenKind::RBRACE))
		{
			...
			do
			{
				...
				if (key->kind != AstKind::IDENTIFIER)
					REALSIX_SCRIPT_LOG_ERROR(key->tagToken, "Struct object require key must be a valid identifier.");

				Consume(TokenKind::COLON, "Expect ':' after struct object's key.");
				Expr *value = ParseExpr();
		...
		Consume(TokenKind::RBRACE, "Expect '}' after struct object.");
		return structExpr;
	}
	...
	{
		auto token = Consume(TokenKind::AMPERSAND, "Expect '&'.");
	...
	Expr *Parser::ParseConditionExpr(Expr *prefixExpr)
	{
		...
		Consume(TokenKind::QUESTION, "Expect '?'.");
		conditionExpr->trueBranch = ParseExpr(Precedence::CONDITION);
		Consume(TokenKind::COLON, "Expect ':' in condition expr");
	...
	Expr *Parser::ParseIndexExpr(Expr *prefixExpr)
	{
		...
		Consume(TokenKind::LBRACKET, "Expect '['.");
	...
	Expr *Parser::ParseCallExpr(Expr *prefixExpr)
	{
		...
		Consume(TokenKind::LPAREN, "Expect '('.");
	...
	{
		auto token = Consume(TokenKind::BANG, "Expect '!'");
	...
	{
		Consume(TokenKind::ELLIPSIS, "Expect '...'");
	...
	std::pair<Expr *, Expr *> Parser::ParseDestructuringAssignmentExpr()
	{
		...
		do
		{
			...
			if (varDescExpr->name->kind == AstKind::VAR_ARG) // check if has var arg
			{
				varArgCount++;
				if (varArgCount > 1)
					REALSIX_SCRIPT_LOG_ERROR(varDescExpr->tagToken, "only 1 variable arg decl is available in var declaration.");
			}
		...
		Consume(TokenKind::RBRACKET, "Expect ']' destructuring assignment expr.");
			...
			for (int32_t i = 0; i < arrayExpr->elements.size(); ++i)
			{
				if (((VarDescExpr *)arrayExpr->elements[i])->name->kind == AstKind::VAR_ARG)
				{
					if (i < arrayExpr->elements.size() - 1)
						REALSIX_SCRIPT_LOG_ERROR(arrayExpr->elements[i]->tagToken, "variable arg decl must be located at the end of destructing assignment declaration.");
				}
			}
		}
		...
		if (IsMatchCurTokenAndStepOnce(TokenKind::EQUAL))
		{
			...
			if (value->kind == AstKind::ARRAY)
			{
				...
				else if (grad > 0)
				{
					...
					REALSIX_SCRIPT_LOG_ERROR(GetCurToken(), "variable less than value.");
			}
	...
	std::vector<Expr *> Parser::ParseSwitchOrMatchConditionExprList(Expr* expr)
	{
		...
		do
		{
			...
			auto caseCondition = new InfixExpr(GetCurToken(), "==", expr, valueCompareExpr);
	...
	Token *Parser::Consume(TokenKind kind, StringView errMsg)
	{
		if (mSkippingConsumeTokenKindStack.empty() || kind != mSkippingConsumeTokenKindStack.back())
		{
			...
			REALSIX_SCRIPT_LOG_ERROR(token, "{}", errMsg);
		}
		return nullptr;
	}

	Token *Parser::Consume(const std::vector<TokenKind> &kinds, StringView errMsg)
	{
