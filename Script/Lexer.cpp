...
namespace RealSix::Script
{
	...
	void Lexer::ScanToken()
	{
		...
		else if (c == TEXT("."))
		{
			if (IsMatchCurCharAndStepOnce(TCHAR('.')))
			{
				if (IsMatchCurCharAndStepOnce(TCHAR('.')))
					...
					REALSIX_LOG_ERROR_WITH_LOC(mCurPos, TEXT("Unknown literal:'..',did you want '.' or '...'?"));
			}
			else
		...
		else
		{
			...
			else
			{
				...
				REALSIX_LOG_ERROR_WITH_LOC(mCurPos, TEXT("Unknown literal:") + literal);
			}
		}
	}
	...
	bool Lexer::IsLetter(CHAR_T c)
	{
		if (!isascii(c))
			return true;
		return (c >= TCHAR('A') && c <= TCHAR('Z')) || (c >= TCHAR('a') && c <= TCHAR('z')) || c == TCHAR('_');
	}
	bool Lexer::IsLetterOrNumber(CHAR_T c)
	{
	...
	void Lexer::Number()
	{
		...
		if (IsMatchCurCharAndStepOnce(TCHAR('.')))
		{
			if (IsNumber(GetCurChar()))
				...
				REALSIX_LOG_ERROR_WITH_LOC(mCurPos, TEXT("The character next to '.' in a floating number must be in [0-9] range or a single 'f' character."));
		}
