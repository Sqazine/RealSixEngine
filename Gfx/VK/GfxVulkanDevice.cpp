...
namespace RealSix
{
	...
	void GfxVulkanDevice::SelectPhysicalDevice()
	{
		...
		{
			REALSIX_LOG_ERROR(TEXT("No Vulkan physical device found!"));
		}
		else if (mPhysicalDeviceSpecificationList.size() == 1)
		{
			mSelectedPhysicalDeviceIndex = 0;
			REALSIX_LOG_INFO(TEXT("Only one Vulkan physical device found, using it."));
		}
		else
		{
		...
		REALSIX_LOG_INFO(TEXT("Selected Vulkan physical device {}: {}, Type: {}, API Version: {}, Driver Version: {}"),
	...
	void GfxVulkanDevice::CheckInstanceValidationLayersIsSatisfied()
	{
		...
		{
			const char *layerName = gInstanceValidationLayers[i];
			...
			if (!layerFound)
			{
				REALSIX_LOG_ERROR(TEXT("Missing Vulkan Validation Layer:{}"), layerName);
			}
		}
	}
#endif
	...
	void GfxVulkanDevice::CheckRequiredInstanceExtensionsIsSatisfied(const std::vector<const char *> &extensions)
	{
		for (const char *extensionName : extensions)
		{
			...
			if (!isFound)
			{
				REALSIX_LOG_ERROR(TEXT("Missing Vulkan Instance Extension:{}"), extensionName);
			}
		}
	}
	...
	{
		REALSIX_LOG_INFO(TEXT("Physical Devices infos:"));

		for (int32_t i = 0; i < mPhysicalDeviceSpecificationList.size(); ++i)
		{
			const auto &spec = mPhysicalDeviceSpecificationList[i];
			REALSIX_LOG_INFO(TEXT("Physical Device {}: {}, Type: {}, API Version: {}, Driver Version: {}"),
	...
	uint32_t GfxVulkanDevice::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
	{
		...
		REALSIX_LOG_ERROR(TEXT("failed to find suitable memory type!"));
	...
	VkFormat GfxVulkanDevice::FindSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		...
		REALSIX_LOG_ERROR(TEXT("failed to find supported format!"));
