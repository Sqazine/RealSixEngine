...
namespace RealSix
{
    ...
    IGfxCommandBuffer *GfxVulkanCommandBuffer::BindRasterPipeline(IGfxRasterPipeline *pipeline)
    {
        ...
        auto vulkanShader = static_cast<GfxVulkanVertexRasterShader *>(pipeline->GetShader());
        vulkanShader->Flush();

        auto sets = vulkanShader->GetDescriptorSetList();
        if (!sets.empty())
    ...
    IGfxCommandBuffer *GfxVulkanCommandBuffer::BindComputePipeline(IGfxComputePipeline *pipeline)
    {
        ...
        auto sets = vulkanComputeShader->GetDescriptorSetList();
        if (!sets.empty())
